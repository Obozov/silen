    ...
    (() => {
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            menunav.addEventListener("click", (event => {
                ...
                if (!clickedItem || clickedItem.classList.contains("header-active")) return;
                const activeItems = menunav.querySelectorAll(".menu__item.header-active");
                activeItems.forEach((item => item.classList.remove("header-active")));
                clickedItem.classList.add("header-active");
                setTimeout((() => delActive(clickedItem)), 3e3);
                document.querySelector(".menu__body").classList.remove("_active");
            }));
            function delActive(itemActive) {
                itemActive.classList.remove("header-active");
            }
            const header = document.querySelector(".header");
            const first = document.querySelector(".first");
            window.addEventListener("scroll", (function() {
                const firstBlockBottom = first.offsetTop + first.offsetHeight;
                if (window.pageYOffset > firstBlockBottom) header.classList.add("header__active"); else header.classList.remove("header__active");
            }));
            ...
            const grayPhotos = document.querySelectorAll(".grayphoto");
            const photoOverlay = document.querySelector(".photo-overlay");
            const enlargedPhoto = document.querySelector(".enlarged-photo img");
            grayPhotos.forEach((grayPhoto => {
                grayPhoto.addEventListener("click", (function(e) {
                    const clickedImgSrc = e.target.getAttribute("src");
                    enlargedPhoto.setAttribute("src", clickedImgSrc);
            ...
            const blagPhotos = document.querySelectorAll(".blagphoto");
            const photoOverlay2 = document.querySelector(".photo-overlay2");
            const enlargedPhoto2 = document.querySelector(".enlarged-photo2 img");
            blagPhotos.forEach((blagPhoto => {
                blagPhoto.addEventListener("click", (function(e) {
                    const clickedImgSrc2 = e.target.getAttribute("src");
                    enlargedPhoto2.setAttribute("src", clickedImgSrc2);
                    photoOverlay2.style.display = "flex";
                }));
            }));
            photoOverlay2.addEventListener("click", (function() {
                photoOverlay2.style.display = "none";
            }));
            const docPhotos = document.querySelectorAll(".docphoto");
            const photoOverlay3 = document.querySelector(".photo-overlay3");
            const enlargedPhoto3 = document.querySelector(".enlarged-photo3 img");
            docPhotos.forEach((docPhoto => {
                docPhoto.addEventListener("click", (function(e) {
                    const clickedImgSrc3 = e.target.getAttribute("src");
                    enlargedPhoto3.setAttribute("src", clickedImgSrc3);
                    photoOverlay3.style.display = "flex";
                }));
            }));
            photoOverlay3.addEventListener("click", (function() {
                photoOverlay3.style.display = "none";
            }));
            const videoBtn = document.querySelectorAll(".video-item");
            const videoBg = document.querySelector(".video__bg");
            const videoYT = document.querySelectorAll(".videoYT");
            document.addEventListener("click", videoActive);
            let isVideoActive = false;
            function videoActive(event) {
                const clickedVideoItem = event.target.closest(".video-item");
                if (clickedVideoItem) {
                    const videoIndex = Array.from(videoBtn).indexOf(clickedVideoItem);
                    videoBg.classList.add("videoActive");
                    videoYT.forEach(((element, index) => {
                        if (index === videoIndex) element.classList.add("videoActive"); else element.classList.remove("videoActive");
                    }));
                    isVideoActive = true;
                } else {
                    videoBg.classList.remove("videoActive");
                    videoYT.forEach((element => {
                        element.classList.remove("videoActive");
                    }));
                    if (isVideoActive) location.reload();
                    isVideoActive = false;
                }
            }
        }));
